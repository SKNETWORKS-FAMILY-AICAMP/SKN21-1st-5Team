...
| :--- | :--- |
| **팀 레이서 정보** | 레이서 팀 정렬하고 레이서들의 일정한 시각화 제공을 위해  `detail > card_view`를 모듈화하여 레이서 정보를 제공. |
| **레이서 비교** | `st.session_state`을 활용해 선택한 레이서들끼리의 성적 비교 데이터를 제공. |
| **검색 결과 조회** | \*\*`st.text_input`\*\*로 검색한 레이서의 성적 데이터를 제공. 검색 정규화로 \*\*`re.fullmatch`\*\* 영어만 가능하도록 설정. |
| **데이터 분석** | 레이서별 **성적 데이터**를 \*\*`st.metric`\*\*로 평균값에비해 up/down 시각화하여 경쟁 구도를 한눈에 파악. |
| **UI/UX 편의성** | `st.navigation`를 통해 주요 설정(메뉴 전환)을 간편하게 조작할 수 있도록 구현. |
...
| **병탁** | 짧은 시간에 프로젝트 구조 잡고 몇 번의 리팩토링으로 어디 내놔도 손색없는 프레임웍을 만든 가람님께 찬사롤 보냅니다. 개발을 처음 해 보는 성진, 현우님도 마침내 크롤링 해 내는 게 참으로 대견합니다. 무궁한 발전을 기원합니다. |
